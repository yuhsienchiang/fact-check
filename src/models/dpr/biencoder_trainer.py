...
class BiEncoderTrainer:
    def __init__(self, model: BiEncoder = None, batch_size: int = 64) -> None:
    ...
    def train(
        ...
        for epoch in range(max_epoch):
            ...
            for index_batch, sample_batch in enumerate(train_dataloader):
                ...
                query_input_ids = sample_batch.query.input_ids.squeeze(1).to(
                    self.device
                )
                query_attn_mask = sample_batch.query.attn_mask.squeeze(1).to(
                    self.device
                )
                # evidence inputs - reshape and move to gpu
                evid_input_ids = torch.flatten(sample_batch.evid.input_ids, 0, 1).to(
                    self.device
                )
                evid_attn_mask = torch.flatten(sample_batch.evid.attn_mask, 0, 1).to(
                    self.device
                )
                ...
                query_vector, evid_vector = self.model(
    ...
    def cosine_similarity(self, query_vec: T, evidence_vec: T):
        ...
        return (
            torch.mm(
                normalized_query_vec, normalized_evidence_vec.transpose(dim0=0, dim1=1)
            )
            * 10.0
        )

    def negative_likelihood_loss(
        self, query_vector: T, evidence_vector: T, is_positive: T
    ):
        similarity_func = self.select_similarity_func(self.model.similarity_func_type)

        # calculate similarity score
        similarity_score = similarity_func(
            query_vec=query_vector, evidence_vec=evidence_vector
        )

        positive_mask = self.create_positive_mask(
            is_positive=is_positive, shape=similarity_score.shape
        ).to(self.device)
