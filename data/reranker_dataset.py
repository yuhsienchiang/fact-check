...
class RerankerDataset(Dataset):
    ...
    def __getitem__(self, idx) -> RerankerSample:
        ...
        positive_evidence_text = self.clean_text(
            data["evidence"], lower_case=self.lower_case
        )

        negative_evidenct_sample = self.evidences_data.sample(
            n=1, random_state=self.rand_seed
        )
        negative_evidenct_text = self.clean_text(
            negative_evidenct_sample["evidences"].to_list()[0]
        )
