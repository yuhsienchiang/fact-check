...
class ClassifierTrainer:
    def __init__(self, classifier: Classifier = None, batch_size: int = 64) -> None:
    ...
    def train(
        ...
        for epoch in range(max_epoch):
            ...
            for index_batch, sample_batch in enumerate(train_dataloader):
                ...
                text_sequences_input_ids = text_sequences.input_ids.squeeze(1).to(
                    self.device
                )
                text_sequences_token_type_ids = (
                    text_sequences.segments.squeeze(1).to(self.device)
                    if text_sequences.segments is not None
                    else None
                )
                text_sequences_attention_mask = text_sequences.attn_mask.squeeze(1).to(
                    self.device
                )
                labels = sample_batch.query_label.to(self.device)

                logit = self.classifier(
                    input_ids=text_sequences_input_ids,
                ...
                del (
                    ...
                    loss,
                    labels,
                    text_sequences_input_ids,
                    text_sequences_attention_mask,
                )
